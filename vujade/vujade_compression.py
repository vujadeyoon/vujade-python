...
class Zip(object):
    ...
    def _check_validation(self, _spath_base: str, _targets: List[str]) -> None:
        for _idx, _target, in enumerate(_targets):
            path_target = path_.Path(_spath=os.path.join(_spath_base, _target))
            if path_target.path.exists() is False:
                raise FileNotFoundError('{}'.format(path_target.str))

    def compress(self, _spath_zip: str, _spath_base: str, _targets: List[str], _level: int = zipfile.ZIP_DEFLATED) -> None:
            ...
            if type_target is True:
                with zipfile.ZipFile(path_zip.str, 'w', _level) as f:
                    for _idx, _target in enumerate(_targets):
                        ...
                        f.write(_target)
                        for _idy, _a in enumerate(path_target.path.rglob(pattern='*')):
                            b = str(_a).replace(os.getcwd(), '.')
                            f.write(b)
                self._cd_cwd()
            else:
